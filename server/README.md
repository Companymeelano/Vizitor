...
### راه‌سرور واقعی
- **تک‌سرور**: وب‌سرویس (IIS) و دیتابیس SQL هر دو روی **یک ماشین** هستند: **`37.143.147.19`**
  - وب‌سرویس (API): `http://37.143.147.19:8731/server/api.php`
  - دیتابیس: SQL Server روی `37.143.147.19:1433` — دیتابیس **`atiran2`** با کاربر `AdminAn`
- اسکریپت را **فقط روی همین سرور** اجرا کنید؛ چون دیتابیس روی همین ماشین است، بخش‌های تنظیمات SQL (TCP/IP و Mixed Mode) به‌صورت **محلی** اعمال می‌شوند — ری‌استارت SQL فقط **با تأیید شما** و با حفظ خودکار سرویس‌های وابسته انجام می‌شود (در ساعت کاری از `-NoSqlRestart` یا پاسخ N استفاده کنید)
- ⚠️ پورت 1433 **نباید** در فایروال عمومی باز باشد — API فقط به‌صورت **محلی (127.0.0.1)** به دیتابیس وصل می‌شود؛ تنها پورت عمومیِ لازم، 8731 است
...
10. تست ورود به `atiran2` با کاربر config (در صورت نیاز، ساخت/فعال‌سازی لاگین به‌صورت خودکار)
11. **تست سرتاسری**: ping با کلید ✔ / رد درخواست بدون کلید (۴۰۱) ✔ / مسدود بودن config.php ✔ + گزارش تعداد رکورد جداول
12. بررسی دسترس‌پذیری `http://37.143.147.19:8731` از بیرون
...
```bash
# تست اتصال
curl -H "X-Api-Key: MILANO-VIZITOR-2026" "http://37.143.147.19:8731/server/api.php?action=ping"

# کاتالوگ
curl -H "X-Api-Key: MILANO-VIZITOR-2026" "http://37.143.147.19:8731/server/api.php?action=catalog" | head

# مشتریان
curl -H "X-Api-Key: MILANO-VIZITOR-2026" "http://37.143.147.19:8731/server/api.php?action=customers" | head
```
