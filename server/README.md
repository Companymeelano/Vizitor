...
### راه‌سرور واقعی
- وب‌سرویس (IIS): `http://37.143.148.14:8731/server/api.php` — اسکریپت را **فقط روی همین ماشین** اجرا کنید
- دیتابیس: SQL Server روی **`37.143.147.19`** — دیتابیس **`atiran2`** با کاربر `AdminAn`
- ⚠️ اسکریپت به سرور ۱۴۷.۱۹ به‌هیچ‌عنوان دست نمی‌زند (نه رجیستری، نه سرویس)؛ فقط اتصال TCP:1433 و ورود با کاربر SQL را **می‌خواند و تست** می‌کند. پورت 1433 باید روی فایروال آن ماشین فقط برای وب‌سرور (۱۴۸.۱۴) باز باشد
