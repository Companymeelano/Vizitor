...
## سؤال باز: کدام دیتابیس؟ + جدول‌های اقلام پیش‌فاکتور (۲۰۲۶-۰۹-۱۸)
...
⚠️ تا وقتی معلوم نشود ERP کدام دیتابیس را «زنده» می‌داند، اپ به هیچ‌کدام قطعی بسته
نمی‌شود و **کد نوشتن سطرهای اقلام فعال نمی‌شود** (چون بدنهٔ `Edit_sail_pish` هم هنوز
نیامده). برای همین `sql/08_which_db.sql` ساخته شد — **راه ۵** در راهنما: یک بار با
`Meelano` انتخاب‌شده و یک بار با `Atiran14050603` اجرا شود؛ ستون `last_write` نشان
می‌دهد دیتابیس زنده کدام است.
