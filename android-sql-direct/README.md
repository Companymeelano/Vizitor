...
## ۹) انتخاب دیتابیس از روی لیست سرور (نسخهٔ ۱.۲ — پیاده‌شده)

مرحلهٔ «دیتابیس حسابداری را انتخاب کن» حالا داخل خود برنامه است:

| فایل | افزوده |
|---|---|
| `SqlConnectionManager.kt` | `suspend fun listDatabases(s: DbSettings): Result<List<String>>` — با همان کاربر/رمزِ واردشده به `master` وصل می‌شود و فقط `SELECT name FROM sys.databases WHERE database_id > 4 AND state = 0 ORDER BY name` را می‌خواند. هیچ چیزی ساخته/تغییر نمی‌شود و رمز جایی log نمی‌شود. |
| `VizitorViewModel.kt` | `dbOptions`، `dbListLoading` و `loadDatabases(settings)` — پیام موفقیت/خطا با همان `_toast` موجود به کاربر نشان داده می‌شود. |
| `SettingsScreen.kt` | دکمهٔ **«دریافت لیست دیتابیس‌ها»** زیر فیلد نام دیتابیس؛ نتیجه به‌صورت چیپ‌های قابل انتخاب می‌آید و با انتخاب هر کدام همان نام در فیلد دیتابیس می‌نشیند. |

پچ آماده برای اعمال روی پروژهٔ اندروید: `patches/phase3-database-picker.patch`

جریان کامل در برنامه:

```
آدرس سرور + پورت 1433 → کاربر و رمز دیتابیس (دسترسی کامل)
      → «دریافت لیست دیتابیس‌ها» → انتخاب دیتابیس حسابداری (مثل Meelano)
      → «تست اتصال SQL» (و بررسی سلامت جدول‌های پیش‌فاکتور)
      → ورود ویزیتور با نام کاربری/کلمهٔ عبور خودش در dbo.sys_users
